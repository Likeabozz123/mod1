...
import xyz.gamars.mod1.core.world.feature.config.FeatureUtil;
import xyz.gamars.mod1.core.world.feature.config.Mod1TreeConfig;
import xyz.gamars.mod1.core.world.feature.config.SaplingData;
import xyz.gamars.mod1.core.world.feature.config.noise.fastnoise.FastNoise;

import javax.annotation.Nullable;
import java.util.*;

public abstract class AbstractTreeFeature <TFC extends Mod1TreeConfig> extends Feature<TFC> {
    ...
    public AbstractTreeFeature(Codec<TFC> configCodec) {
        super(configCodec);
    }
    ...
    public void placeTrunk(BlockPos startPos, Mod1TreeConfig config, Random random, Set<BlockPos> blockSet, ISeedReader reader, BlockPos pos, MutableBoundingBox boundingBox) {
    ...
    public void placeBranch(BlockPos startPos, Mod1TreeConfig config, Random random, Set<BlockPos> blockSet, ISeedReader reader, BlockPos pos, MutableBoundingBox boundingBox) {
        pos = getTransformedPos(config, startPos, pos);
        if (canLogPlaceHere(reader, pos)) {
            this.setFinalBlockState(startPos, config, blockSet, reader, pos, config.getTrunkProvider().getState(random, pos), boundingBox);
        }
    ...
    public void placeLeaves(BlockPos startPos, Mod1TreeConfig config, Random random, Set<BlockPos> blockSet, ISeedReader reader, BlockPos pos, MutableBoundingBox boundingBox) {
        pos = getTransformedPos(config, startPos, pos);
        if (isAir(reader, pos)) {
    ...
    public void placeLeaves(BlockPos startPos, Mod1TreeConfig config, Random random, ISeedReader reader, int x, int y, int z, MutableBoundingBox boundingBox, Set<BlockPos> blockPos) {
    ...
    public static BlockPos getTransformedPos(Mod1TreeConfig config, BlockPos startPos, BlockPos pos) {
    ...
    /*public void etherBulbs(BlockPos startPos, Mod1TreeConfig config, Random random, Set<BlockPos> blockSet, ISeedReader reader, BlockPos pos, MutableBoundingBox boundingBox) {
        pos = getTransformedPos(config, startPos, pos);
        this.setFinalBlockState(startPos, config, blockSet, reader, pos, BlockInit.ETHER_BULB.defaultBlockState().setValue(EtherBulbsBlock.AGE, random.nextInt(4)), boundingBox);
    }*/

    /*public void ArisianBloomBranch(BlockPos startPos, Mod1TreeConfig config, Random random, Set<BlockPos> blockSet, ISeedReader reader, BlockPos pos, MutableBoundingBox boundingBox, Direction direction) {
        pos = getTransformedPos(config, startPos, pos);
        this.setFinalBlockState(startPos, config, blockSet, reader, pos, BlockInit.ARISIAN_BLOOM_BRANCH.defaultBlockState().setValue(ImpariusMushroomBranchBlock.FACING, direction), boundingBox);
    }*/

    /*public void baobabFruit(BlockPos startPos, Mod1TreeConfig config, Random random, Set<BlockPos> blockSet, ISeedReader reader, BlockPos pos, MutableBoundingBox boundingBox) {
        pos = getTransformedPos(config, startPos, pos);
        if (isAir(reader, pos) && reader.getBlockState(pos.above()).getBlock() == BlockInit.BAOBAB_LEAVES) {
            this.setFinalBlockState(startPos, config, blockSet, reader, pos, BlockInit.BAOBAB_FRUIT_BLOCK.defaultBlockState().setValue(BaobabFruitBlock.AGE, random.nextInt(4)), boundingBox);
        }
    }*/

    public void placeNetherTrunk(BlockPos startPos, Mod1TreeConfig config, Random random, Set<BlockPos> blockSet, ISeedReader reader, BlockPos pos, MutableBoundingBox boundingBox) {
        pos = getTransformedPos(config, startPos, pos);
    ...
    public void placeNetherBranch(BlockPos startPos, Mod1TreeConfig config, Random random, Set<BlockPos> blockSet, ISeedReader reader, BlockPos pos, MutableBoundingBox boundingBox) {
        pos = getTransformedPos(config, startPos, pos);
        if (canLogPlaceHereNether(reader, pos)) {
            this.setFinalBlockState(startPos, config, blockSet, reader, pos, config.getTrunkProvider().getState(random, pos), boundingBox);
        }
     ...
     */
    public static boolean isDesiredGroundwDirtTag(IWorldGenerationBaseReader reader, BlockPos pos, Mod1TreeConfig config) {
    ...
    public static boolean isDesiredGroundwNetherTags(IWorldGenerationBaseReader reader, BlockPos pos, Mod1TreeConfig config) {
        if (config.isPlacementForced())
            return true;
    ...
    public static boolean isDesiredGroundwEndTags(IWorldGenerationBaseReader reader, BlockPos pos, Mod1TreeConfig config) {
        if (config.isPlacementForced())
            return true;
    ...
    public static boolean isDesiredGroundwSandTag(IWorldGenerationBaseReader reader, BlockPos pos, Mod1TreeConfig config) {
        if (config.isPlacementForced())
            return true;
    ...
    public void buildTrunk(ISeedReader reader, Mod1TreeConfig config, Random random, BlockPos operatingPos, int downRange) {
    ...
    public void setDisk(ISeedReader world, Random random, BlockPos pos, Mod1TreeConfig config) {
    ...
    public final void setFinalBlockState(BlockPos startPos, Mod1TreeConfig config, Set<BlockPos> changedBlocks, IWorldWriter worldIn, BlockPos pos, BlockState blockState, MutableBoundingBox boundingBox) {
    ...
    public void setBlockStateWithoutUpdates(BlockPos startPos, Mod1TreeConfig config, IWorldWriter worldWriter, BlockPos blockPos, BlockState blockState) {
        worldWriter.setBlock(blockPos, blockState, 18);
    }

    public void setBlockStateWithoutUpdates(BlockPos startPos, Mod1TreeConfig config, IWorldWriter worldWriter, BlockPos blockPos, BlockState blockState, int flags) {
    ...
    public boolean place(ISeedReader worldIn, ChunkGenerator generator, Random rand, BlockPos pos, TFC config) {
        ...
        if (worldIn.getLevel().dimension() == World.OVERWORLD && !Mod1.ENABLE_OVERWORLD_TREES) {
            return false;
        }
